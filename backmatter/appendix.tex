...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/2Auth/(NEW) Auth-Class-Diagram.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/3Profile/(NEW) Profile-Class-Diagram.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/4Resume/(NEW) Resume-Class-Diagram.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/1Public/(NEW) Public-Class-Diagram.pdf}
...
The Back Office module is hosted under \texttt{api-internal}. Figures~\ref{diagram:class-job-moderation}, \ref{diagram:class-user-management}, and \ref{diagram:class-audit-log} present its class diagrams.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/7BackOffice/(NEW) JobModeration-Class-Diagram.pdf}
	\caption{Job Moderation Module Class Diagram.}
	\label{diagram:class-job-moderation}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/7BackOffice/(NEW) UserManagement-Class-Diagram.pdf}
	\caption{User Management Module Class Diagram.}
	\label{diagram:class-user-management}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/7BackOffice/(NEW) AuditLog-Class-Diagram.pdf}
	\caption{Audit Log Module Class Diagram.}
	\label{diagram:class-audit-log}
\end{figure}

\subsection{Back Office Worker UI}

On the presentation side, the back office is realized as a role-guarded route group under \texttt{/admin} that the Application Shell exposes only to users with the \texttt{ADMIN} role. \texttt{AdminLayout} provides a persistent sidebar that navigates between three page areas mirroring the modules above: Job Moderation, a list of postings filterable by status and defaulting to \texttt{PENDING\_APPROVAL}, with a detail page carrying the approve, reject, and delete actions; Users, a searchable account list with a detail page offering ban and unban actions; and Audit Logs, a read-only listing of administrative actions.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1.25\linewidth]{diagrams/exported/5JobSeeker/SD-JobSeeker01-Apply-For-Jobs.pdf}
...
\chapter{Domain Enumerations}
\label{appendix:domain-enumerations}

This chapter lists the domain enumerations referenced by name in the entity-relationship diagrams and the module designs. The complete value set of each enumeration is provided here for reference.

\section{JobStatus}

Referenced in Chapter~\ref{chapter:system-design} as the type of the job posting's \texttt{status} attribute (Figure~\ref{diagram:erd-company-job}).

\begin{tabular}{|l|p{9.8cm}|}
	\hline
	\textbf{Value} & \textbf{Description} \\
	\hline
	PENDING\_APPROVAL & A newly created or edited posting awaiting moderation approval. \\
	\hline
	ACTIVE & Approved by an administrator and publicly visible. \\
	\hline
	REJECTED & Rejected by an administrator during moderation. \\
	\hline
	CLOSED & Closed by the employer before the deadline. \\
	\hline
	EXPIRED & Past the application deadline; no longer accepting applications. \\
	\hline
	DELETED & Soft-deleted by the employer; the record is retained for history. \\
	\hline
\end{tabular}

\section{ApplicationStatus}

Referenced in Chapter~\ref{chapter:system-design} as the type of the application's \texttt{status} attribute (Figure~\ref{diagram:erd-job-application}).

\begin{tabular}{|l|p{9.8cm}|}
	\hline
	\textbf{Value} & \textbf{Description} \\
	\hline
	SUBMITTED & Initial state when a job seeker applies. \\
	\hline
	UNDER\_REVIEW & The employer has acknowledged the application and is reviewing it. \\
	\hline
	ACCEPTED & The employer has accepted the application. \\
	\hline
	REJECTED & The employer has rejected the application. \\
	\hline
	WITHDRAWN & The job seeker has withdrawn the application. \\
	\hline
\end{tabular}