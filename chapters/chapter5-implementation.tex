...
\paragraph{applyJob}
The core of the module is the \texttt{applyJob} method, which guards every business rule from the use case specification before any row is written. The checks run in order: the caller must be a job seeker; the referenced job must exist and be \texttt{ACTIVE}, otherwise the posting is unreachable; the chosen resume must exist, be not deleted, and belong to the caller; and no application may already exist for the same user and job pair. Only when all checks pass is an \texttt{Application} row created with the initial status \texttt{SUBMITTED}. The following excerpt shows the early guard sequence.

\begin{verbatim}
assertJobSeeker(currentUser);

const job = await prisma.job.findUnique({
  where: { id: requestBody.jobId },
  select: { id: true, status: true },
});
if (!job)                    throw new HttpError(404, "Job not found.");
if (job.status !== "ACTIVE") throw new HttpError(400,
  "Job posting is no longer available.");

const resume = await prisma.resume.findFirst({
  where: { id: requestBody.resumeId, deletedAt: null },
});
...
const application = await prisma.application.findUnique({
  where: { userId_jobId: { userId: currentUser.id, jobId: job.id } },
});
if (application) throw new HttpError(409,
...
This method returns only the caller's applications. Because an application relates to a job, and a job relates to its company, a single Prisma query joins those relations and maps each row to a flattened list item carrying the application status and submission date together with the job and company summary -- the fields the tracking page displays.
...
The \texttt{api-internal/job-moderation/} module defines the approval, rejection, and deletion operations for moderators. At the time of writing its frontend is not yet implemented and the approval operation is still being wired, so job postings reach \texttt{ACTIVE} through moderation only partially; this is noted as in-progress work rather than covered as a finished feature.
