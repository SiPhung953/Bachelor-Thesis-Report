...
These principles are applied consistently across all modules of the system. They are illustrated in detail for the two featured modules, Job Application and Employer Job Management; the design of the remaining modules is documented in Appendix~\ref{appendix:supplementary-design}.
...
The system architecture is documented using the C4 model, which describes a software system at progressively narrower levels of zoom: Context, Container, and Component.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.85\linewidth]{diagrams/exported/C4/(NEW) C1-Context-Diagram-crop.pdf}
	\caption{System Context Diagram.}
	\label{diagram:c1-context}
\end{figure}

As shown in Figure~\ref{diagram:c1-context}, three actors interact with the platform: the Job Seeker browses and applies for jobs, the Employer publishes and manages job postings, and the System Administrator moderates users and platform content.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\linewidth]{diagrams/exported/C4/(NEW) C2-Container-Diagram-crop.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/C4/(NEW) C3-Component-Diagram-FE-crop.pdf}
	\caption{Frontend Component Diagram.}
	\label{diagram:c3-frontend}
\end{figure}

As shown in Figure~\ref{diagram:c3-frontend}, the feature components are organized primarily by the roles they serve. Job Seeker, Employer, Back Office Worker functionality is grouped separately, while Job Discovery is shared across access contexts and can be used by both Guest and authenticated users.

Two shared components support the feature layers. The Application Shell and Router manages navigation and role-based access, while the Shared UI Kit provides reusable interface elements and common loading and error states.

Every feature component communicates with the backend exclusively through a centralized API Client, a generated SDK that wraps REST calls, injects the Bearer token, handles 401 interception, and parses responses. No feature component makes direct HTTP requests; the API Client is the single outbound integration point.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/C4/(NEW) C3-Component-Diagram-BE-crop.pdf}
	\caption{Backend Component Diagram.}
	\label{diagram:c3-backend}
\end{figure}

As illustrated in Figure~\ref{diagram:c3-backend}, the backend components are organized primarily by functional responsibility and user role. Public-facing functionality is separated from role-specific components, while Job Seeker, Employer, and Back Office components are grouped according to their respective workflows.

A shared infrastructure layer supports the role-specific components by providing common services such as authentication, mailing, file storage, and audit logging.

All components access PostgreSQL through a single Prisma Client instance, providing a consistent data access mechanism and avoiding service-to-service API calls within the application.
...
The remainder of this section details the entity groups that back the two featured modules - the company and job posting domain and the job application domain. The entities supporting the remaining modules are described in Appendix~\ref{appendix:supplementary-erd}.
...
An employer may own at most one company, and each company belongs to exactly one employer. Similarly, an employer may create multiple job postings, but each job posting is created by exactly one employer. Each job posting belongs to exactly one company, while a company may publish any number of job postings. The \texttt{status} attribute represents the current state of a job posting through the \texttt{JobStatus} enumeration.
...
A job seeker may upload multiple resumes and submit multiple applications, but each resume and each application belongs to exactly one job seeker. Each application is submitted for exactly one job posting, while a job posting may receive any number of applications. A resume may be attached to multiple applications, but each application references exactly one resume. The application's \texttt{status} attribute records where the application stands in the review process through the \texttt{ApplicationStatus} enumeration.


\section{Backend Design}
\label{c4:section:backend-design}
This section details the internal design of the Backend API container introduced in Section~\ref{c4:section:system-architecture}. It first describes the project structure and shared infrastructure common to all modules, then presents the two featured business modules following the same template: an introduction, a class diagram, and a design explanation. The class diagrams of the remaining modules are presented in Appendix~\ref{appendix:supplementary-design}.
...
\begin{table}[H]
	...
	\begin{tabular}{|p{3.2cm}|p{10.8cm}|}
		...
		logging &
		Provides transactional audit logging services and message formatting for system event tracking. \\
		\hline
		
		generated &
		Contains files generated by tsoa and Prisma during the build. \\
		\hline
...
\begin{itemize}
	...
	\item Role Guards: helpers such as \texttt{assertJobSeeker} and \texttt{assertEmployer} reject requests from users who lack the required role, applied as the first step of every service method. The guards are always paired with the tsoa \texttt{@Security} decorator form defense in depth.
	...
	\item AuditLogger: records a structured entry for a system event inside the same database transaction as the mutation it accompanies.
\end{itemize}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/5JobSeeker/(NEW) JobApplication-Class-Diagram.pdf}
	\caption{Job Application Module Class Diagram.}
	\label{diagram:class-job-application}
\end{figure}

\subsubsection{Design Explanation}
Following the thin controller, rich service principle, ApplicationController exposes three REST endpoints and delegates each directly to a matching ApplicationService method, without containing business logic itself. This separation keeps HTTP adaptation concerns - request parsing, response shaping, and error mapping - distinct from the business rules that determine how an application can be created, listed, or withdrawn.

ApplicationService is the only class in the module that owns business logic and the only class that accesses the database, doing so through the shared Prisma Client. It operates on the User, Job, Resume, and Application entities whose relationships are detailed in Figure~\ref{diagram:erd-job-application}. The service depends on two shared infrastructure components: AssertRole, and AuditLogger. These components are documented in Section~\ref{c4:subsection:shared-backend-components}.

Authentication and role authorization are enforced at two levels. At the controller boundary, tsoa's \texttt{@Security("jwt", ["JOB\_SEEKER"])} decorator causes the framework to verify the JWT and confirm the caller's role before the controller method is entered. The service then re-asserts the same role via AssertRole, providing defense in depth: business methods remain protected regardless of how they are invoked. Each service method corresponds to a single use case described in Chapter~\ref{chapter:system-analysis}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/6Employer/(NEW) CompanyProfileCreation-Class-Diagram.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/6Employer/(NEW) JobPostingManagement-Class-Diagram.pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{diagrams/exported/6Employer/(NEW) EmployerApplicationManagement-Class-Diagram.pdf}
	\caption{Employer Application Management Class Diagram.}
	\label{diagram:class-employer-application}
\end{figure}

\subsubsection{Design Explanation}
The Employer Job Management Module is decomposed into three cohesive controller and service pairs, each owning a distinct subdomain of the employer workflow. EmployerCompanyController and EmployerCompanyService handle company profile management; JobManagementController and JobManagementService manage the lifecycle of job postings; and EmployerApplicationController and EmployerApplicationService cover the review of incoming applications. This separation keeps each pair focused on a single business concern and allows it to evolve independently.

Across all three pairs the same structural pattern is applied: each controller is annotated with tsoa's \texttt{@Security("jwt", ["EMPLOYER"])} decorator at the class level, while exposing REST endpoints and delegating each directly to a matching service method. Each service is the sole owner of business rules and the only class in its pair that accesses the database through the shared Prisma Client. Every service method depends on AssertRole and AuditLogger, which are described in Section~\ref{c4:subsection:shared-backend-components}.

Company Profile Management takes no company identifier in its operations because the employer-company relationship is one-to-one: the owning employer is resolved from the authenticated user. Job Posting Management's own methods can only move a posting into \texttt{PENDING\_APPROVAL}, \texttt{CLOSED}, or \texttt{DELETED}, so a posting cannot become publicly visible without first passing through this external moderation gate. Employer Application Management exposes operations for the employer-driven status transitions that complement the job-seeker-initiated \texttt{SUBMITTED} and \texttt{WITHDRAWN} transitions defined in the Job Application Module. Together the three pairs implement the complete round trip of a recruitment event studied in Chapter~\ref{chapter:system-analysis}.
