...
The challenges described in Chapter \ref{chapter:introduction} motivate a centralized online recruitment platform. In this system, a job seeker can search for jobs, keep a personal profile, upload resumes, submit applications, and follow their progress. An employer can register a company, publish job postings, and review applicants. A back-office role moderates content and manages accounts. Putting these operations in one system replaces the fragmented workflow of the current market with a single consistent interface.

The system's functionality is split into clear modules by business responsibility:

\begin{itemize}
	\item \textbf{Job Discovery} allows guests to search active job postings and view company profiles.
	\item \textbf{Authentication} covers registration, login, logout, and password recovery.
	\item \textbf{Profile Management} lets users maintain personal information, job preferences, avatar, and account password.
	\item \textbf{Resume Management} lets job seekers upload and delete resumes used in applications.
	\item \textbf{Job Application} lets a job seeker apply to an active posting, track submitted applications, and withdraw them while still eligible.
	\item \textbf{Employer Job Management} lets an employer manage a company profile, run the full lifecycle of a job posting, and review the applications a posting receives.
	\item \textbf{Administration} supports moderating job postings and managing users and logs.
\end{itemize}

Because the platform stores personal data and lets users modify shared records, access is controlled. Registration and login protect each user's account, and role-based permissions decide what each user may do. A job seeker manages their own profile and applications; an employer manages only their own company, postings, and candidates; a back-office worker moderates platform content.

This thesis develops the platform in full but examines two flows in depth: the \textbf{Job Application} flow on the job seeker's side and the \textbf{Employer Job Management} flow on the employer's side. These two flows form the round trip of a recruitment event — a job seeker submits an application, and an employer reviews it — and are therefore used to illustrate the analysis, design, and implementation in the remaining chapters.
...
Use case analysis models how users interact with the platform and shows the functional scope from each user role's perspective. The platform serves three primary roles: Job Seeker, Employer, and Administrator. Each role performs different functions and uses a different subset of the system.

\subsection{Shared Use Cases}

To keep the model compact, the actors are organized with UML generalization. Signed-in users are represented by the generalized \textbf{Authenticated User} actor, and visitors are represented by the \textbf{Guest} actor.
...
The following subsections present the role-specific use cases. In line with the scope defined in Section \ref{c3:section:proposed-system}, the Job Seeker and Employer roles are examined in detail as the two featured modules of this thesis; the Back Office Worker use cases are presented in Appendix \ref{appendix:back-office-use-cases}.

\subsubsection{Job Seeker Use Cases}

The Job Seeker is the main user on the demand side of the platform: this actor looks for jobs, keeps a profile, and takes part in recruitment through job applications.
...
Figure \ref{diagram:uc-job_seeker} shows the Job Seeker's capabilities. Personal profile maintenance is part of the authenticated account functions; the recruitment-specific behavior of this actor is the \textbf{Job Application} flow: applying to an active posting with an uploaded resume, tracking submitted applications, and withdrawing an application while it is still in its initial state.
...
Figure \ref{diagram:uc-employer} shows the \textbf{Employer Job Management} module, which covers the complete recruitment lifecycle on the employer's side: creating a company profile, publishing and managing job postings (edit, close, re-open, delete), listing applications received for a posting, and moving each application through review until it is accepted or rejected.

\section{Use Case Specification}

The use case diagrams above give an overview of the system's functional scope. This section specifies the main use cases of the two featured modules in detail. Specifications for the remaining modules are provided in the appendix.
